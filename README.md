...
1. Renaming the controller from `SuperTwisting` to `MyPlugin`. In a shell (Git Bash on Windows, replace sed with gsed on macOS):

```bash
sed -i -e's/SuperTwisting/MyPlugin/g' `find . -type f`
git mv src/SuperTwisting.cpp src/MyPlugin.cpp
git mv src/SuperTwisting.h src/MyPlugin.h
git mv etc/SuperTwisting.in.yaml etc/MyPlugin.in.yaml
```
